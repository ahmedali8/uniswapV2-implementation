...
export default function shouldBehaveLikeMint(): void {
  context("when the beneficiary is the zero address", function () {
    it("reverts", async function () {
      await expect(
        this.contracts.token.connect(this.signers.user).mint(AddressZero, Zero)
  ...
  context("when the beneficiary is not the zero address", function () {
    context("when the mint results into an overflow", function () {
      beforeEach(async function () {
        await this.contracts.token
          ...
          .mint(this.signers.user.address, toBN("1"));
      });
      ...
      it("reverts", async function () {
        await expect(
          this.contracts.token
            .connect(this.signers.user)
            .mint(this.signers.user.address, MaxUint256)
        ).to.be.reverted;
      });
    });
    ...
    context("when the mint does not result into an overflow", function () {
      ...
      it("increases the balance of the beneficiary", async function () {
        await this.contracts.token
          .connect(this.signers.user)
          .mint(this.signers.user.address, mintAmount);
        expect(await this.contracts.token.balanceOf(this.signers.user.address)).to.equal(
          mintAmount
        );
      });

      it("increases the total supply", async function () {
        const preTotalSupply: BigNumber = await this.contracts.token.totalSupply();
        await this.contracts.token
          .connect(this.signers.user)
          .mint(this.signers.user.address, mintAmount);
        const postTotalSupply = await this.contracts.token.totalSupply();
